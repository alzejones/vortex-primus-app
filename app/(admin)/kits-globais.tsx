// ============================================================
// (admin)/kits-globais.tsx — Administração de Kits Globais Herbalife
// NÍVEL 0 (Admin/Master) — Apenas usuários em platform_admins
// ============================================================
...
export default function AdminKitsGlobais() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      console.error('Erro ao carregar kits globais:', e);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  async function saveKit() {
    ...
    try {
      if (editingKit) {
        ...
        // CRIAR NOVO KIT GLOBAL (trainer_id sempre NULL)
        const { data: newKit, error: insertErr } = await supabase
          .from('herbalife_kits')
          .insert({ 
            trainer_id: null, // SEMPRE NULL para kits globais
            name: kitName.trim(), 
            kit_type: kitType, 
            default_price: price, 
            is_redemption_only: isRedemptionOnly, 
    ...
    <View style={{ flex: 1, backgroundColor: T.bg }}>
      <View style={{ padding: 16, paddingBottom: 0 }}>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 12 }}>
          <Text style={s.pageTitle}>Kits Globais</Text>
          <View style={s.badgeAdmin}>
            <Text style={s.badgeAdminTxt}>ADMIN</Text>
          </View>
        </View>
        <Text style={s.subtitle}>
          Kits disponíveis para todos os trainers da plataforma
        </Text>
      </View>
      ...
      >
        {kits.length === 0 && (
          <Text style={s.empty}>Nenhum kit global cadastrado. Crie o primeiro kit abaixo!</Text>
        )}
        {kits.map((kit) => (
          <View key={kit.id} style={s.kitCard}>
            <View style={{ flex: 1 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                ...
                {kit.is_redemption_only && (
                  <View style={s.badgeRedemption}>
                    <Text style={s.badgeRedemptionTxt}>Resgate</Text>
                  </View>
                )}
              </View>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                <Text style={s.kitPrice}>{brl(Number(kit.default_price))}</Text>
                <Text style={s.kitType}>
                  {kit.kit_type === 'fechado' ? '📦 Fechado' : '💧 Doses'}
                </Text>
              </View>
            </View>
      ...
      <View style={s.fabContainer}>
        <TouchableOpacity style={s.fab} onPress={openCreateModal}>
          <Text style={s.fabTxt}>+ Novo Kit Global</Text>
        </TouchableOpacity>
      </View>
      ...
      <Modal visible={modalOpen} animationType="slide" transparent>
        <View style={s.modalBg}>
          <View style={s.modalBox}>
            <ScrollView>
              <Text style={s.modalTitle}>{editingKit ? 'Editar Kit Global' : 'Novo Kit Global'}</Text>
...
const s = StyleSheet.create({
  ...
  badgeAdminTxt: { color: '#ef4444', fontSize: 10, fontWeight: '800' },
  badgeRedemption: {
