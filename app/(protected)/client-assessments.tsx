...
export default function ClientAssessments() {
  ...
  const renderGridInput = (label: string, key: keyof typeof form, unit: string) => (
    <View style={{ marginBottom: 12 }}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center' }}>
        ...
        />
        <Text style={{ fontSize: 12, color: '#888', marginLeft: 4 }}>{unit}</Text>
      </View>
    </View>
  );
        ...
        <KeyboardAvoidingView style={{ flex: 1, backgroundColor: "#fff" }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
          ...
          <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 100 }}>
            ...
            <View style={styles.card}>
              <Text style={styles.cardTitle}>1. Medidas Básicas</Text>
              <View style={styles.row}>
                <View style={{ flex: 1, marginRight: 4 }}>{renderGridInput("Peso", "weight", "kg")}</View>
                <View style={{ flex: 1, marginLeft: 4 }}>{renderGridInput("Altura", "height", "cm")}</View>
              </View>
              <View style={styles.row}>
                <View style={{ flex: 1, marginRight: 4 }}>{renderGridInput("Cintura", "waist", "cm")}</View>
                <View style={{ flex: 1, marginLeft: 4 }}>{renderGridInput("Quadril", "hip", "cm")}</View>
              </View>
            </View>

            {/* 🔴 O BOTÃO MÁGICO DE AVALIAÇÃO À DISTÂNCIA ESTÁ AQUI */}
            <TouchableOpacity 
              style={{ backgroundColor: '#eff6ff', padding: 16, borderRadius: 12, borderWidth: 1, borderColor: '#3b82f6', marginBottom: 16, marginTop: 8, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', shadowColor: "#3b82f6", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.2, shadowRadius: 5, elevation: 3 }}
              onPress={calculateRemoteAssessment}
            >
              <Text style={{ fontSize: 20, marginRight: 8 }}>🪄</Text>
              <View>
                <Text style={{ color: '#1d4ed8', fontWeight: '900', fontSize: 15 }}>Auto-Preencher via IA</Text>
                <Text style={{ color: '#2563eb', fontSize: 11 }}>Calcula Gordura, Músculo e Metabolismo</Text>
              </View>
            </TouchableOpacity>

            <View style={styles.card}>
              <Text style={styles.cardTitle}>2. Tronco e Membros</Text>
              <View style={styles.row}>
                <View style={{ flex: 1, marginRight: 4 }}>{renderGridInput("Peitoral", "chest", "cm")}</View>
                <View style={{ flex: 1, marginLeft: 4 }}>{renderGridInput("Abdômen", "abdomen", "cm")}</View>
              </View>
              <View style={styles.row}>
                <View style={{ flex: 1, marginRight: 4 }}>{renderGridInput("Braço Dir", "arm_right", "cm")}</View>
                <View style={{ flex: 1, marginLeft: 4 }}>{renderGridInput("Braço Esq", "arm_left", "cm")}</View>
              </View>
              <View style={styles.row}>
                <View style={{ flex: 1, marginRight: 4 }}>{renderGridInput("Coxa Dir", "thigh_right", "cm")}</View>
                <View style={{ flex: 1, marginLeft: 4 }}>{renderGridInput("Coxa Esq", "thigh_left", "cm")}</View>
              </View>
              <View style={styles.row}>
                <View style={{ flex: 1, marginRight: 4 }}>{renderGridInput("Pant. Dir", "calf_right", "cm")}</View>
                <View style={{ flex: 1, marginLeft: 4 }}>{renderGridInput("Pant. Esq", "calf_left", "cm")}</View>
              </View>
            </View>

            <View style={styles.card}>
              <Text style={styles.cardTitle}>3. Bioimpedância</Text>
              <View style={styles.row}>
                <View style={{ flex: 1, marginRight: 4 }}>{renderGridInput("% Gordura", "body_fat", "%")}</View>
                <View style={{ flex: 1, marginLeft: 4 }}>{renderGridInput("% Músculo", "muscle_mass_percentage", "%")}</View>
              </View>
              <View style={styles.row}>
                <View style={{ flex: 1, marginRight: 4 }}>{renderGridInput("Índice Visceral", "body_fat_index", "pts")}</View>
                <View style={{ flex: 1, marginLeft: 4 }}>{renderGridInput("Idade Metabólica", "metabolic_age", "anos")}</View>
              </View>
              <View style={styles.row}>
                <View style={{ flex: 1 }}>{renderGridInput("Taxa Metabólica Basal", "basal_metabolic_rate", "kcal")}</View>
              </View>
            </View>
