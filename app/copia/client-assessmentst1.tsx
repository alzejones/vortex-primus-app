...
async function fetchHistory() {
  try {
    ...
    // 1. Busca os dados básicos do Cliente (Nome, Objetivo, etc.)
    ...
    // 2. Busca as Avaliações e já traz os dados de Antropometria junto (Join)
    ...
    // Atualiza o estado das avaliações para alimentar os gráficos e a lista
    setAssessments(assessmentsData || []);

  } catch (error: any) {
    console.error("Erro ao carregar histórico:", error.message);
    Alert.alert("Erro", "Não foi possível carregar as avaliações deste aluno.");
  } finally {
    setLoading(false);
  }
}
...
// 2. Função que faz o trabalho pesado
async function fetchClientAndAssessments() {
  try {
    setLoading(true);

    // Busca dados básicos do cliente (para o cabeçalho/gráficos)
    const { data: clientData, error: clientError } = await supabase
      .from("clients")
      .select("*")
      .eq("id", clientId)
    ...
    // Busca o histórico de avaliações físico-químicas e antropometria
    // Ajuste o join conforme sua estrutura (geralmente usamos o .select com as tabelas relacionadas)
