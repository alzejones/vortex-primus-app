...
DECLARE
  extracted_name text;
    new_trainer_id uuid;
    BEGIN
      -- Clientes convidados não criam registro em trainers
        IF (NEW.raw_user_meta_data->>'role') = 'client' THEN
            RETURN NEW;
              END IF;

                -- Extração do nome com fallbacks
                                      ...
                                      IF extracted_name IS NULL OR extracted_name = '' THEN
                                          extracted_name := 'Novo Treinador';
                                            END IF;
                                              ...
                                              -- Cria o trainer e captura o ID gerado
                                                new_trainer_id := gen_random_uuid();
                                                  INSERT INTO public.trainers (id, user_id, name, email)
                                                    VALUES (new_trainer_id, NEW.id, extracted_name, NEW.email);

                                                      -- Associa automaticamente ao plano Teste (gratuito)
                                                        INSERT INTO public.trainer_subscriptions (
                                                            id,
                                                                trainer_id,
                                                                    plan_id,
                                                                            ...
                                                                            is_active,
                                                                                start_date,
                                                                                    started_at
                                                                                          ...
                                                                                          gen_random_uuid(),
                                                                                              new_trainer_id,
                                                                                                  '9d8a50e0-007a-4e5f-ab1c-3641629204a7',  -- plano Teste
                                                                                                      'active',
                                                                                                          true,
                                                                                                              CURRENT_DATE,
                                                                                                                  now()
...
CREATE TABLE IF NOT EXISTS "public"."assessment_photos" (
    ...
    "label" "text" DEFAULT 'outro'::"text",
    "created_at" timestamp with time zone DEFAULT "now"(),
    CONSTRAINT "assessment_photos_label_check" CHECK (("label" = ANY (ARRAY['frente'::"text", 'costas'::"text", 'lateral_dir'::"text", 'lateral_esq'::"text", 'outro'::"text"])))
);


ALTER TABLE "public"."assessment_photos" OWNER TO "postgres";
...
ALTER TABLE ONLY "public"."assessment_photos"
...
CREATE INDEX "idx_assessment_photos_assessment_id" ON "public"."assessment_photos" USING "btree" ("assessment_id");



CREATE INDEX "idx_assessment_photos_client_id" ON "public"."assessment_photos" USING "btree" ("client_id");
...
ALTER TABLE ONLY "public"."assessment_photos"
    ADD CONSTRAINT "assessment_photos_assessment_id_fkey" FOREIGN KEY ("assessment_id") REFERENCES "public"."physical_assessments"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."assessment_photos"
    ADD CONSTRAINT "assessment_photos_client_id_fkey" FOREIGN KEY ("client_id") REFERENCES "public"."clients"("id");



ALTER TABLE ONLY "public"."assessment_photos"
    ADD CONSTRAINT "assessment_photos_trainer_id_fkey" FOREIGN KEY ("trainer_id") REFERENCES "public"."trainers"("id");
...
ALTER TABLE ONLY "public"."trainers"
    ADD CONSTRAINT "trainers_plan_id_fkey" FOREIGN KEY ("plan_id") REFERENCES "public"."plans"("id");
...
CREATE POLICY "meal_log_auth" ON "public"."meal_log" TO "authenticated" USING (true) WITH CHECK (true);
...
CREATE POLICY "meal_plan_foods_auth" ON "public"."meal_plan_foods" TO "authenticated" USING (true) WITH CHECK (true);
...
CREATE POLICY "meal_plan_meals_auth" ON "public"."meal_plan_meals" TO "authenticated" USING (true) WITH CHECK (true);
...
CREATE POLICY "meal_plans_auth" ON "public"."meal_plans" TO "authenticated" USING (true) WITH CHECK (true);
...
CREATE POLICY "trainer_can_manage_own_photos" ON "public"."assessment_photos" USING (("trainer_id" IN ( SELECT "trainers"."id"
   FROM "public"."trainers"
  WHERE ("trainers"."user_id" = "auth"."uid"()))));
...
GRANT ALL ON TABLE "public"."assessment_photos" TO "anon";
GRANT ALL ON TABLE "public"."assessment_photos" TO "authenticated";
GRANT ALL ON TABLE "public"."assessment_photos" TO "service_role";
